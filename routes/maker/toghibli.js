...
router.get('/', async (req, res) => {
    const { url } = req.query;
    const fixedPrompt = `Anda adalah seorang ahli seni visual dan kritikus film animasi. 
Tugas: Analisis foto subjek ini dan deskripsikan secara rinci bagaimana tampilan foto ini jika dirender ulang dalam gaya visual Studio Ghibli.

Fokuskan pada elemen-elemen berikut:
1.  **Gaya Karakter**: Bagaimana subjek akan digambar? (Mata besar, ekspresi lembut, rambut realistis/sederhana ala Ghibli).
2.  **Latar Belakang**: Deskripsikan detail lingkungan (pepohonan rimbun, langit biru cerah dengan awan bergulung, bangunan dengan detail arsitektur sederhana namun indah).
3.  **Warna & Pencahayaan**: Tone warna (cerah, pastel, hangat, natural), sumber cahaya (sinar matahari menembus dedaunan, cahaya lembut dari jendela).
4.  **Atmosfer**: Rasa nostalgia, keajaiban, ketenangan, atau melankolis yang khas Ghibli.
5.  **Detail Spesifik**: Elemen kecil yang bisa ditambahkan (misal: daun berguguran, bunga liar, makhluk kecil tersembunyi).

Output harus berupa deskripsi naratif yang imajinatif dan mendetail, seolah-olah Anda sedang menuliskan skenario visual untuk adegan Ghibli.`; 
