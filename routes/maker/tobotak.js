...
router.get('/', async (req, res) => {
    const { url } = req.query;
    const fixedPrompt = `Anda adalah ahli manipulasi gambar digital dan anatomi manusia. 
Tugas: Analisis foto subjek ini secara teliti.
Instruksi:
1. Identifikasi seluruh area rambut pada kepala subjek (Hairline, crown, dan bagian samping).
2. Berikan koordinat bounding box atau segmentasi area yang harus dihilangkan untuk membuat subjek terlihat botak plontos (bald).
3. Deskripsikan bentuk tengkorak subjek berdasarkan struktur wajahnya untuk memastikan hasil editan botak terlihat natural.
4. Berikan rekomendasi skin-tone untuk area kulit kepala yang tertutup rambut agar sesuai dengan warna kulit wajah subjek.
5.hilangkan rambut dari kepala subjek 

Output harus teknis, mendetail, dan berbasis koordinat agar bisa diproses oleh tim editor/engine manipulasi gambar.`; 
